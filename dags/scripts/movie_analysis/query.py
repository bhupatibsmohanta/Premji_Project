...
def movie_analysis():
    ...
    genres = ['unknown', 'Action', 'Adventure', 'Animation', "Children's", 'Comedy', 'Crime', 'Documentary', 'Drama',
              ...
              'Western']
    item_file = open("dags/scripts/movie_analysis/ml-100k/u.item", "r", encoding='iso-8859-1')
    for x in item_file:
    ...
    print("\n\nTask1: mean age of users in each occupation")
    print(tabulate(avg_ages, headers='keys', tablefmt='grid'))

    conn = sqlite3.connect(':memory:')
    c = conn.cursor()
    
    survey_data.to_sql('survey_data', conn, index=False, if_exists='replace')
    item_data.to_sql('item_data', conn, index=False, if_exists='replace')
    ...
    limit 20
    """
    top_20_movies = pd.read_sql_query(query2, conn)

    print("\n\nTask2: Names of top 20 highest rated movies. (at least 35 times rated by Users)")
    ...
    user_data.to_sql('user_data', conn, index=False, if_exists='replace')
    item_xplode.to_sql('item_xplode', conn, index=False, if_exists='replace')
    topgenres = pd.read_sql_query(query3, conn)

    print("\n\nTask3: Top genres rated by users of each occupation in every age-groups. age-groups can be defined as 20-25, 25-35, 35-45, 45 and older")
